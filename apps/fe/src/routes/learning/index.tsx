...
const Learning = () => {
  ...
  const progression = userInfo?.progression;
  const studyStats = userInfo?.studyStats;
    ...
    <div className="flex min-h-screen justify-center bg-pale-blue p-8">
      <div className="w-full max-w-5xl space-y-6 md:space-y-8">
        ...
        <section>
          ...
          <div className="scrollbar-hide flex gap-4 overflow-x-auto pb-4 md:grid md:grid-cols-2 md:gap-3 md:overflow-visible lg:grid-cols-4">
            {topicOptions.map((topic) => {
              ...
              return (
                <div
                  ...
                  className={`group min-w-55 cursor-pointer rounded-xl border-2 p-4 transition-all active:scale-95 md:min-w-0 md:active:scale-100 ${
                    isSelected
        ...
        {/* 난이도 설정 */}
        <section
          className={`mb-8 rounded-2xl border border-gray bg-white p-5 shadow-sm transition-all duration-700 ease-in-out md:p-8 ${selectedTopic !== null ? `translate-y-0 opacity-100` : `pointer-events-none translate-y-10 opacity-0`}`}
        >
          <h3 className="mb-4 text-lg font-bold md:text-xl">난이도 설정</h3>
          <div className="flex w-full rounded-lg bg-gray p-1">
            {difficultyOptions.map((diff) => (
              <button
                key={diff.value}
                onClick={() => setSelectedDifficulty(diff.value)}
                className={`flex-1 rounded-md py-3 text-sm transition-all active:scale-95 md:py-2 md:active:scale-100 ${
        ...
        >
          ...
          <div className="z-10 mb-6 w-full md:mb-0 md:w-auto">
            ...
            <h2 className="mb-1 text-lg font-bold md:text-xl">오늘의 AI 튜터가 준비되었습니다.</h2>
